/**
* @Author: meteor
* @Date:   2016-09-22T10:37:25+08:00
* @Email:  1056874343@qq.com
* @Last modified by:   yanke
* @Last modified time: 2016-10-13T17:03:01+08:00
*/
...
import {Button,Header,Loading,Tip,Textarea} from '../components'
...
class BillDetail extends Component{
  ...
  _submit(opcode){
    //订金结算完单据中没有新车
    if(this.state.dataSource.subscription.subscription_ok && this.state.dataSource.items && !this.state.dataSource.items[1]){
    ...
    if(opcode == '8192'){
      ...
      .then(res=>{
        this.setState({LoadIcon:false})
        Actions.pop({refresh: {message: 'refresh page'}});
      })
    }
...
//判断items里是否有数据
      ...
      return (
        <TouchableHighlight key={key} style={styles.bd_Bar} underlayColor='#f2f2f2' onPress={this.state.changeAvilable && !item.finance_ok ? () => this._editSuit(key, item, list, data) : null}>
          <View style={{flexDirection: 'row'}}>
            <View style={[styles.md_itemsItem, {flex: 1, paddingRight: Utils.normalize(5)}]}>
              <View style={[styles.md_itemsTextBox,{alignItems:'center'}]}>
                <Text style={styles.md_itemsText}>{(item.goods_info?item.goods_info+'-':'')+item.goods_name}</Text>
              </View>
              <View>
  ...
  render(){
    ...
    btns = btns.map((item,key)=>{
      if(!this.state.hasGoods && item[0] != '8192'){
        return null;
      }
      if(item[0] == '8'){
        //是否勾选预先支付订金 是否为订单
              ...
              <View>
                ...
                (this.state.dataSource.items  && this.state.data.car.length) || this.state.dataSource.subscription.subscription_ok ?
                 <View>
                   ...
                   <TouchableHighlight underlayColor='#f2f2f2' style={[styles.bd_Bar, {  flexDirection: 'row', backgroundColor: '#fff'}]} onPress={this.state.changeAvilable ? () => {this._subscription()} : null}>
                     <View style={{flexDirection: 'row', flex: 1, alignItems: 'center',backgroundColor: '#fff'}}>
                       <View style={{flex: 1,marginTop: 6}}>
                         <View style={{flexDirection: 'row', marginBottom: Utils.normalize(3)}}>
                           ...
                           <Text style={{color: '#999999', fontSize: Utils.normalize(14), paddingTop: Utils.normalize(1), textAlign: 'right',flex: 1}}>{ !data.subscription.subscription_has  ? '未填写' : ''}</Text>
                         </View>
                         {
                                        ...
                                        }
                                        {
                                          ...
                                          <View style={[styles.row,{paddingTop: Utils.normalize(14),paddingBottom: Utils.normalize(14)}]}>
                                            <View style={{flex: 1,flexDirection: 'row',marginLeft: Utils.normalize(43)}}>
                                              <Button pattern={{outLine:'smallBorderBtn',text:'smallBorderBlue'}} onPress={this._changBought.bind(this)} value="查看已购车辆"></Button>
                                            </View>
                                            <View style={{flex: 1,flexDirection: 'row',marginRight: Utils.normalize(43),justifyContent: 'flex-end',}}>
                                              <Button pattern={{outLine:'smallBorderBtn',text:'smallBorderBlue'}} onPress={()=>this._addNewCar()} value="录入新车辆"></Button>
                                            </View>
                                          </View>
                                          : <View/>
                                        }
                                        {
                                          data.subscription.subscription_ok ?
                                          <View>
                                            <View>
                                              <TouchableHighlight style={{backgroundColor:'#fff'}} underlayColor='#f2f2f2' onPress={()=>this._usedCar(this)}>
                                                <View style={[styles.row,{height:39,justifyContent:'center',alignItems:'center'}]}>
                ...
                <View style={{backgroundColor:'#fff',marginTop:9}}>
                  <TouchableHighlight style={{backgroundColor:'#fff'}} underlayColor='#f2f2f2' onPress={()=>{this._remarkDetail(data.common.des)}} >
                    <View style={[styles.row,{height:39,justifyContent:'center',alignItems:'center'}]}>
                      <Text style={[styles.md_itemsTextCar,{width:60,flex:0}]}>单据备注</Text>
