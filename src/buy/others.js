...
* @Last modified time: 2016-11-14T14:45:00+08:00
*/

/**
 * Created by Nic on 16/8/29.
 */
...
class Others extends Component{
  ...
  _renderRow(msg,sectionID,rowId){
    if(msg != 'Ot'){
      return(
        <View style={[ot_styles.renderView]}>
          <View style={{ flexDirection: 'column',flex:4}}>
            <Text style={[ot_styles.label,{fontSize:Utils.normalize(15)}]} >{msg.name}</Text>
            <View style={[ot_styles.label,{flexDirection: 'row'}]}>
              <View style={{flex:1,flexDirection: 'row'}}>
                <Text style={[ot_styles.grayColor]}>指导价:</Text>
                <Text style={[ot_styles.redColor,{paddingLeft:12}]}>￥{msg.price ? Utils.oFixed(msg.price,2,true) : Utils.oFixed(0,2,true)}</Text>
              </View>
              <View style={{flex:1,flexDirection: 'row'}}>
                <Text style={[ot_styles.grayColor]}>供应商:</Text>
                <Text style={[ot_styles.redColor,{paddingLeft:12}]}>{msg.sup_name ? msg.sup_name : null}</Text>
              </View>
            </View>
            <View style={[ot_styles.label,{flexDirection: 'row',width: Utils.width-60}]}>
          ...
          <View style={{ flexDirection: 'column',flex:1,justifyContent:'flex-end'}}>
            <TouchableOpacity style={ot_styles.smRedButtom} onPress={()=>this._addCart(msg)}>
              <Text style={ot_styles.smButtomText}> {msg.active?'已购买':'购买'}</Text>
            </TouchableOpacity>
          </View>
        </View>
      )
    ...
    return(
      <View>
        <ListView
          ...
          style={[ot_styles.listContainer,(Object.keys(this.state.dataSource._dataBlob.s1).length==1?{backgroundColor:'#EFEFEF'}:{})]}
        ...
        {this.state.showCartList?
          <Animated.View style={{position:'absolute',top:this.state.aniList,backgroundColor: 'transparent',width:Utils.width,height:Utils.height}}>
            ...
            <View style={{backgroundColor:'rgba(0, 0, 0, 0.4)'}}>
              <View style={{backgroundColor:'#fff',justifyContent:'center',alignItems:'center',height:Utils.normalize(50)}}>
            ...
            <ListView style={{height:Utils.height*0.6-Utils.normalize(50),backgroundColor:'#fff'}}
      ...
      {this.state.backToTop ?
        <TouchableOpacity activeOpacity={1} onPress={() => this.refs._listView.scrollTo({y: 0, animated: true})} style={ot_styles.backToTop}>
